...
void just_bmi_calculator() {
    ...
    do {
        ...
        std::cin >> weight;
        if (weight <= 0) {
            std::cout << "Weight must be greater than zero. Please try again." << std::endl;
        }
    } while (weight <= 0);
    ...
    do {
        ...
        std::cin >> height;
        if (height <= 0) {
            std::cout << "Height must be greater than zero. Please try again." << std::endl;
        }
    } while (height <= 0);

    double bmi = calculate_bmi(weight, height); // Calculate BMI
...
void calculate_bmi_for_friend() {
    ...
    while (std::getline(inputFile, line)) {
        ...
        char grade;
        iss >> name >> surname >> age >> weight >> height >> grade;

        // Check if details match the entered name and surname
        if (name == nameToCalculate && surname == surnameToCalculate) {
...
void main_menu() {
    ...
    // Main menu loop
    while (!exitLoop) {
        std::cout << "\n";// To separete and make it easy to read
        ...
        case 4:
            ...
            break;
        case 5:
              bmi_calculator_choice(); // Calculate BMI for a friend
            break;
        case 6:
